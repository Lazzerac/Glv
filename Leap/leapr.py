...
# Get screen resolution
        ...
        Listnr.scalex=s.width_in_pixels/200
        Listnr.scaley=s.height_in_pixels/200
        Listnr.rootw=s.root
...
# Ok, done
        ...
        #     s_line2 = ser.read(size=20)
        #     s2 = struct.unpack('hhhhhhhhhH',s_line2)
        #     keystroke = s2[9]
        #     if s2[9] !=8:
        #         print s2[9]
        #         if (keystroke):
        #             if keystroke == 1:
        #                 Listnr.lClickFlag = True
        #             elif keystroke == 2:
        #                 Listnr.rClickFlag = True
        #             elif keystroke == 3:
        #                 pass
        #             elif keystroke == 4:
        #                 pass
        #             elif keystroke == 5:
        #                 if Listnr.lClickFlag == True:
        #                     ext.xtest.fake_input(Listnr.display, X.ButtonPress, 1)
        #                     Listnr.display.flush()
        #                     Listnr.prevframes[0:Listnr.nframes]=Listnr.frames[0:Listnr.nframes]
        #                     Listnr.rotationlock = currentframe.timestamp
        #                 elif Listnr.rClickFlag == True:
        #                     ext.xtest.fake_input(Listnr.display, X.ButtonPress, 2)
        #                     Listnr.display.flush()
        #                     Listnr.prevframes[0:Listnr.nframes]=Listnr.frames[0:Listnr.nframes]
        #                     Listnr.rotationlock = currentframe.timestamp
        #             elif keystroke == 6:
        ...
        #                 pass
        #             elif keystroke == 7:
        #                 Listnr.releaseFlag = True
        #                 Listnr.holdCount = 0
        #
        #         if  Listnr.releaseFlag == True:
        #             if Listnr.lClickFlag == True:
        #                 ext.xtest.fake_input(Listnr.display, X.ButtonRelease, 1)
        #                 Listnr.display.flush()
        #                 Listnr.prevframes[0:Listnr.nframes]=Listnr.frames[0:Listnr.nframes]
        #                 Listnr.rotationlock = currentframe.timestamp
        #                 Listnr.releaseFlag = False
        #             elif Listnr.rClickFlag == True:
        #                 ext.xtest.fake_input(Listnr.display, X.ButtonRelease, 2)
        #                 Listnr.display.flush()
        #                 Listnr.prevframes[0:Listnr.nframes]=Listnr.frames[0:Listnr.nframes]
        #                 Listnr.releaseFlag = False
...
def main():
    ...
    while 1:
        currentFrameduex = controller.frame()
        s_line = ser.read()
        if struct.unpack('<c',s_line)[0] == '!':
            s_line2 = ser.read(size=20)
            s2 = struct.unpack('hhhhhhhhhH',s_line2)
            ...
            if s2[9] !=8:
                ...
                if (keystroke):
                    if keystroke == 1:
                        listener.lClickFlag = True
                    elif keystroke == 2:
                        listener.rClickFlag = True
                    elif keystroke == 3:
                        pass
                    elif keystroke == 4:
                        pass
                    elif keystroke == 5:
                        if listener.lClickFlag == True:
                            ext.xtest.fake_input(listener.display, X.ButtonPress, 1)
                            listener.display.flush()
                            listener.prevframes[0:listener.nframes]=listener.frames[0:listener.nframes]
                            listener.rotationlock = currentFrameduex.timestamp
                        elif listener.rClickFlag == True:
                            ext.xtest.fake_input(listener.display, X.ButtonPress, 2)
                            listener.display.flush()
                            listener.prevframes[0:listener.nframes]=listener.frames[0:listener.nframes]
                            listener.rotationlock = currentFrameduex.timestamp
                    elif keystroke == 6:
                        listener.holdCount += 1
                        pass
                    elif keystroke == 7:
                        listener.releaseFlag = True
                        listener.holdCount = 0

                if  listener.releaseFlag == True:
                    if listener.lClickFlag == True:
                        ext.xtest.fake_input(listener.display, X.ButtonRelease, 1)
                        listener.display.flush()
                        listener.prevframes[0:listener.nframes]=listener.frames[0:listener.nframes]
                        listener.rotationlock = currentFrameduex.timestamp
                        listener.releaseFlag = False
                    elif listener.rClickFlag == True:
                        ext.xtest.fake_input(listener.display, X.ButtonRelease, 2)
                        listener.display.flush()
                        listener.prevframes[0:listener.nframes]=listener.frames[0:listener.nframes]
                        listener.rotationlock = currentFrameduex.timestamp
                        listener.releaseFlag = False
